...
for ($x=0;$x<$employees_cnt;$x++) {
    ...
    if (($employees_empfullname[$x] == $fullname) || ($fullname == "All")) {
        ...
        $query = "select ".$db_prefix."info.fullname, ".$db_prefix."info.`inout`, ".$db_prefix."info.timestamp, ".$db_prefix."info.notes, 
                  ".$db_prefix."info.ipaddress, ".$db_prefix."employees.employid, ".$db_prefix."punchlist.in_or_out, ".$db_prefix."punchlist.punchitems, ".$db_prefix."punchlist.color
        ...
        while ($row=mysql_fetch_array($result)) {
		    ...
		    echo "                <td nowrap width=5% align=left style='padding-left:10px;font-size:11px;color:#27408b;
		    text-decoration:underline;'>Employee ID</td>\n";
            ...
            echo "                <td nowrap align=right width=5% bgcolor='$row_color' style='padding-right:10px;'>".$row["employid"]."</td>\n";
